...
6. [ ] Web scaffold: React app with Google sign-in and a verification page that calls the ducks routes and shows each response; web stack (S3 bucket, CloudFront distribution, certificate), workflow job, Cloudflare records, CORS response headers on the HTTPS listener. Result: signing in at https://quack.ryt.dev upserts the duck and the page shows it; an expired token gets 401.
7. [ ] Flocks: flocks service and stack: create, list, add member, delete; the same calls on the verification page. Result: from the page, a duck creates, lists, adds a member, and deletes; only the owner can delete; rules from flows/README.md hold.
8. [ ] Messages: messages service and stack: send with publish to the flock topic, history; the same calls on the verification page. Result: from the page, a sent message persists; history returns the latest 50 newest first; non-member send and read get 403.
9. [ ] Real-time: tickets route on ducks, websocket service (redeem ticket, subscribe per flock, push, ping and pong, close at token expiry, unsubscribe on close), websocket stack, /ws listener rule; a socket on the verification page. Result: two browser sessions on the page; a message from one arrives at the other; a non-member session receives nothing.
10. [ ] Web client: the user interface over the verification page's calls: flock list, create and delete, add member, history, live updates, token refresh. Result: two browser sessions at https://quack.ryt.dev exchange messages.
...
| 3.6 Web scaffold         | React scaffold, Google sign-in, verification page, web stack, certificate, Cloudflare records, CORS, workflow job                                              | 2.5      | 3.75    |        |
| 3.7 Flocks               | Service, stack, verification page calls                                                                                                                        | 1.5      | 2.25    |        |
| 3.8 Messages             | Service, stack, publish, verification page calls                                                                                                               | 1.5      | 2.25    |        |
| 3.9 Real-time            | Ticket route, websocket service, subscriptions, close at expiry, /ws rule, stack, socket on the verification page                                              | 3        | 4.5     |        |
| 3.10 Web client          | User interface for every flow, live updates, token refresh                                                                                                     | 1.5      | 2.25    |        |
