...
5. [ ] Ducks: shared token module, fastify verify-and-resolve hook, request log line, shared service construct (task definition, service, target group, listener rule, log group, database role and grants), ducks service (PUT and GET /ducks/me, GET /ducks) and stack. Result: the ducks service is deployed by the workflow and /health answers; PUT /ducks/me with a missing or invalid token gets 401 with an error line in the log group; unit tests pass.
...
| 3.5 Ducks                | Token module, fastify hook, log line, service construct, ducks service and stack, IAM database authentication from a task                                      | 3        | 4.5     |        |
