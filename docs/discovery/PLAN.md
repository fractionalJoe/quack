...
3. [ ] Compute: compute stack (ECS cluster, load balancer, HTTPS listener, certificate, SSM parameters under /quack/compute/), workflow job, Cloudflare validation and api CNAME records. Result: https://api.quack.ryt.dev answers with the listener's default response over a valid certificate; the stack is in CloudFormation.
4. [ ] Fanout: Valkey node in the data stack (replication group with one node, TLS in transit, subnet group, security group, SSM parameters under /quack/data/). Result: the data stack update passes in the workflow and the endpoint parameter exists.
...
| 3.3 Compute              | Compute stack with ALB and certificate, workflow job, Cloudflare records                                                                                       | 1.5      | 2.25    |        |
| 3.4 Fanout               | Valkey node in the data stack                                                                                                                                  | 0.5      | 0.75    |        |
