...
The pond ID is the one row in ponds, inserted by the schema migration and read by each service at start (architecture.md, Ponds).
...
**ponds**, owned by the migration. One row in the MVP, read by every service at start.
