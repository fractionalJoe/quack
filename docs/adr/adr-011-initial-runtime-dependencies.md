...
Which npm packages do the services and the web client start with? The constraint is minimal dependencies, AWS primitives and the standard library over third-party packages (CON-06), against a one-weekend estimate (CON-01). Each slot below is a place where the standard library was weighed against a package.
...
| HTTP server                    | fastify                        | Node http module with an own router; express    | Routing, JSON bodies, schema validation, and a per-request hook for verify and resolve, inside the estimate. The http module costs an hour or two of scaffolding first.                                     |
...
Each package replaces code that would otherwise be written and explained in the repo; the shortest path wins where the package is standard and installs from npm. Native binaries and GitHub installs were rejected in every slot because they complicate the container build and the reader's setup.
