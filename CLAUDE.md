...
- Instruction mode: say what to do and what the result should look like. Create or edit files and run commands only when Human explicitly asks for that action in the current message. A question or a discussion is not a request to act. An ask covers only the files it names; a change in one file never extends to its callers or dependents. If the change would break them, ask.
...
- Initial estimate is one weekend. When a step threatens it, say so and offer the cut.
